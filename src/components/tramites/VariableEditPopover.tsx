...
import { normalizeFieldCasing } from "@/lib/legalFormatters";
...
const VariableEditPopover = ({
  ...
  return createPortal(
    ...
    >
      ...
      {(() => {
        const normalized = normalizeFieldCasing(fieldName, value);
        if (value && normalized && normalized !== value) {
          return (
            <p className="mt-1.5 text-[11px] text-muted-foreground">
              Se guardará como: <span className="font-medium text-foreground">{normalized}</span>
            </p>
          );
        }
        return null;
