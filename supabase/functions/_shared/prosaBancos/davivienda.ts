// ============================================================================
// prosaBancos/davivienda — Redacción canónica INMUTABLE del Banco Davivienda.
//
...
// aprobación explícita del área jurídica + actualización de los tests de
// snapshot `davivienda.natural.test.ts` y `davivienda.juridica.test.ts`.
//
// Normalización estilística (obligatoria — homologada 2026-07-03):
//   - Nombres de personas, cargos y entidades → MAYÚSCULAS SOSTENIDAS.
//   - Fechas y números → prosa notarial vía `numeroConLetras` / `fechaProsa`
//     de `legalProse.ts` (skill formato-texto-numero-notarial).
//   - Sin tabulaciones ni dobles espacios; separador de bloques = " ".
// ============================================================================

import { numeroConLetras, fechaProsa } from "../../process-expediente/legalProse.ts";
...
// Escritura de constitución del banco (dato notarial fijo — no proviene del OCR).
...
/** Devuelve una cédula limpia (solo dígitos). */
function ced(s?: string | null): string {
  return (s ?? "").toString().replace(/\D/g, "");
}
/**
 * Prosa de fecha: prefiere `fecha` (YYYY-MM-DD → helper); si no hay,
 * cae a `fecha_texto` tal cual (ya viene en formato notarial del OCR).
 */
...
/**
 * Descripción de la sociedad apoderada como sub-oración
 * (constituida mediante ..., inscrita en la cámara ..., cambio de razón ...).
 * Devuelve "" si no hay datos mínimos (el classifier ya habrá degradado a null).
 */
...
// ── Router por tipo efectivo (natural | juridica) ──────────────────────────
function isJuridica(ctx: ProsaContext): boolean {
  // El caller siempre pasa un ctx cuyo classifier ya devolvió un tipo definido;
  // la decisión aquí se limita a leer apoderado.tipo del payload confiable.
